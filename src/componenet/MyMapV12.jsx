...
const MyMapV12 = () => {
    ...
    setMapState((prevState) => ({
      ...
      imageIcons: [
        ...prevState.imageIcons,
        {
          ...
          type,
          id: `icon-${nextNumber}`,
          splitterRatio: type === "Splitter" ? "1:2" : null,
        },
      ],
    ...
    if (mapState.isSavedRoutesEditable || !mapState.showSavedRoutes) {
      ...
      if (icon.type === "Splitter") {
        setMapState((prevState) => ({
          ...
          splitterRatio: icon.splitterRatio || "1:2",
          waypointActionPosition: { x, y },
        }));
      } else {
        setMapState((prevState) => ({
    ...
    setMapState((prevState) => {
      ...
      const updatedFiberLines = prevState.fiberLines.map((line) => {
        if (
          ...
          line.from.lat ===
            prevState.imageIcons.find((i) => i.id === iconId)?.lat &&
          line.from.lng ===
            prevState.imageIcons.find((i) => i.id === iconId)?.lng
        ) {
  ...
  const handleStartMarkerDragEnd = (index, e) => {
    ...
    setMapState((prevState) => {
      ...
      if (nearestIcon && nearestIcon.type === "Splitter") {
        const [_, splitNum] = (nearestIcon.splitterRatio || "1:2")
          .split(":")
          .map(Number);
        const maxLines = splitNum + 1; // Allow ratio + 1 lines
        const connectedLines = getConnectedLinesCount(nearestIcon);

        if (
          connectedLines >= maxLines &&
          !isSnappedToIcon(line.from.lat, line.from.lng)
        ) {
          alert(
            `Cannot connect more lines. Splitter limit (${nearestIcon.splitterRatio}) reached.`
          );
          return prevState; // No change if limit exceeded
        }
      }
  ...
  const handleWaypointDragEnd = (lineIndex, waypointIndex, e) => {
    ...
    setMapState((prevState) => {
      const nearestIcon = findNearestIcon(newLat, newLng);
      const updatedFiberLines = prevState.fiberLines.map((line, index) => {
        if (index === lineIndex && line.waypoints) {
          const updatedWaypoints = [...line.waypoints];
          updatedWaypoints[waypointIndex] = nearestIcon
            ? { lat: nearestIcon.lat, lng: nearestIcon.lng }
            : { lat: newLat, lng: newLng };
          return { ...line, waypoints: updatedWaypoints };
        }
      ...
      if (prevState.showSavedRoutes && mapState.isSavedRoutesEditable) {
        updatedSavedPolylines = updatedSavedPolylines.map((line, index) => {
          if (index === lineIndex && line.waypoints) {
            const updatedWaypoints = [...line.waypoints];
            updatedWaypoints[waypointIndex] = nearestIcon
              ? { lat: nearestIcon.lat, lng: nearestIcon.lng }
  ...
  useEffect(() => {
    ...
    setMapState((prevState) => ({
      ...
      imageIcons: savedIcons.map((icon) => ({
        ...icon,
        splitterRatio:
          icon.splitterRatio || (icon.type === "Splitter" ? "1:2" : null),
      })),
    }));
  }, []);
        ...
        if (polyline.id === polylineId) {
          ...
          if (nearestIcon && nearestIcon.type === "Splitter") {
            ...
            if (
              ...
              return polyline; // No change if limit exceeded
            }
          }

          return { ...polyline, [pointType]: newPoint };
        }
        return polyline;
      });
  ...
  const handleSavedPolylineWaypointDragEnd = (polylineId, waypointIndex, e) => {
    ...
    setMapState((prevState) => {
      const updatedSavedPolylines = prevState.savedPolylines.map((polyline) =>
        polyline.id === polylineId && polyline.waypoints
          ? {
              ...polyline,
