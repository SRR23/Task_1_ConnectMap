...
const FiberMapPage = () => {
  ...
  const fetchCables = useCallback(async () => {
    try {
      ...
      // Update savedPolylines and merge with modifiedCables
      setSavedPolylines(polylines);
      setModifiedCables((prev) => {
        const updated = {};
        Object.keys(prev).forEach((cableId) => {
  ...
  const saveNewFiberLines = useCallback(async () => {
    ...
    if (results.every((result) => result)) {
      // Keep cablesToSave in fiberLines until fetchCables updates the state
      return true;
    }
    setFiberLines(cablesToKeep); // Only update if save fails
    return false;
  ...
  const autoSave = useCallback(async () => {
    try {
      ...
      if (devicesSaved && newCablesSaved && modifiedCablesSaved) {
        ...
        // Only clear UI-related states after successful fetch
        ...
        // Clear updatedDevices and fiberLines only after fetch
        setUpdatedDevices([]);
        setFiberLines((prev) =>
          prev.filter((line) => !line.startPortId || !line.endPortId)
        );
        setModifiedCables((prev) => {
          const updated = {};
