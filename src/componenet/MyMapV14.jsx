...
const MyMapV14 = () => {
  ...
  const getAvailableRatios = (splitter) => {
    ...
    return ratios.filter((ratio) => {
      ...
      return ratioNum >= connectedCount; // Include ratios >= current connected lines
    });
  };
  ...
  const removeSelectedLine = () => {
    ...
    setMapState((prevState) => {
      ...
      // Update splitter ratios based on new connected line count
      updatedImageIcons = updatedImageIcons.map((icon) => {
        if (icon.type === "Splitter" && icon.ratioSetTimestamp) {
          const newConnectedCount = (
            isSavedLine ? updatedSavedPolylines : updatedFiberLines
          ).filter(
            (line) =>
          ...
          const currentRatioNum = getRatioNumber(icon.splitterRatio);
          if (newConnectedCount < currentRatioNum && newConnectedCount > 0) {
            const availableRatios = ["1:2", "1:4", "1:8", "1:16", "1:32"];
            const newRatio = availableRatios.find(
              (r) => getRatioNumber(r) >= newConnectedCount
            );
            return { ...icon, splitterRatio: newRatio || "" };
          }
        }
        return icon;
      });
      ...
      updatedSavedIcons = updatedSavedIcons.map((icon) => {
        const updatedIcon = updatedImageIcons.find((i) => i.id === icon.id);
  ...
  const handleSplitterRatioChange = (e) => {
    ...
    if (newRatio === "") return; // Ignore if placeholder is selected
