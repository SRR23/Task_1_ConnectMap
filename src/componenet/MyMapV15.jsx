...
const MyMapV15 = () => {
  ...
  const removeSelectedLine = () => {
    ...
    setMapState((prevState) => {
      ...
      // Update savedIcons to match imageIcons
  ...
  const handleSavedPolylinePointDragEnd = (polylineId, pointType, e) => {
    ...
    setMapState((prevState) => {
      ...
      if (!polylineUpdated) return prevState; // No update needed

      localStorage.setItem("savedPolylines", JSON.stringify(newSavedPolylines));

      // Update fiberLines only if showSavedRoutes is true to keep them in sync
      const newFiberLines = prevState.showSavedRoutes
        ? newSavedPolylines
        : prevState.fiberLines;
