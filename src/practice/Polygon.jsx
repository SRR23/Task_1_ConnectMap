...
const Polygon = () => {
  ...
  }, [drawing, points, isPolygonClosed, polygons, previewIndex, editingIndex, mapCenter]);
  ...
  const handleMouseMove = useCallback((e) => {
    if (drawing) {
      ...
      if (points.length > 0 && !isPolygonClosed && !isDragging) {
        setCurrentMousePosition(currentPos);
      }
    }
  }, [drawing, points.length, isPolygonClosed, isDragging]);
  ...
  const handleEditPolygon = (index) => {
    ...
    setDrawing(true);
    setIsPolygonClosed(true); // Start with closed shape for editing
    setPreviewIndex(null);
      ...
      >
        ...
        >
          ...
          {drawing && points.length > 0 && (
            <>
              {/* During editing or drawing, show only the current shape */}
              {isPolygonClosed ? (
                <GooglePolygon
                  paths={points}
                  options={{
